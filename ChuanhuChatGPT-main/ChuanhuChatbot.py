...
my_api_key = "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"    # 在这里输入你的 API 密钥
...
print("川虎的温馨提示：访问 http://localhost:7880 查看界面")
...
if dockerflag:
    if authflag:
        demo.queue().launch(server_name="0.0.0.0", server_port=7880,auth=(username, password))
    else:
        demo.queue().launch(server_name="0.0.0.0", server_port=7880, share=False)
#if not running in Docker
else:
    demo.queue().launch(share=False) # 改为 share=True 可以创建公开分享链接
    #demo.queue().launch(server_name="0.0.0.0", server_port=7880, share=False) # 可自定义端口
    #demo.queue().launch(server_name="0.0.0.0", server_port=7880,auth=("在这里填写用户名", "在这里填写密码")) # 可设置用户名与密码
